...
# Run the whole batch (not just the selected email) so emails that are the
# same opportunity - e.g. E009/E010, a contact correction - get consolidated
# the same way the CLI (`python -m scr.main`) does, instead of each looking
# independent when viewed one at a time.
all_results = [process_email(e) for e in emails]
all_results, _corrections = apply_contact_corrections(all_results, {e["id"]: e for e in emails})
result = next(r for r in all_results if r.id == selected_id)
...
# --- Linked enquiry (contact correction) ---------------------------------
correction_entry = next((a for a in result.audit_log if a["event"] == "contact_correction"), None)
if correction_entry:
    st.subheader("🔗 Enquiry Ini Terhubung dengan Email Lain")
    details = correction_entry["details"]
    st.info(correction_entry["reason"])
    if "superseded_by" in details:
        st.caption(
            f"Enquiry ini digantikan oleh **{details['superseded_by']}** — tidak ada balasan terpisah "
            f"yang dibuat untuk enquiry ini; balasan sudah dicakup oleh email tersebut."
        )
    else:
        st.caption(
            f"Enquiry ini menggabungkan **{details['merged_with']}** — info kontak di bawah adalah "
            f"versi terkoreksi/terbaru, tapi nilai aslinya tetap tersimpan di tabel riwayat ini."
        )
    history_rows = [
        {
            "Field": h["field"].title(),
            "Nilai": h["value"] or "—",
            "Dari email": h["source_email_id"],
            "Koreksi terbaru?": "✅" if h["is_correction"] else "",
        }
