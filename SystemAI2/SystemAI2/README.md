...
- **`classifier.py`** - ordered keyword rules assign one of: `legal_compliance`, `commercial_enquiry`, `invoice_dispute`, `technical_query`, `logistics_confirmation`, `recruitment`, `partner_engagement`, `internal_alert`, `spam`. `legal_compliance` is checked first, ahead of every other rule, and is always resolved deterministically - it is never deferred to an LLM. For text matching no deterministic rule, `classify_email()` accepts an optional `llm_client` for genuinely ambiguous/low-confidence cases (`method="llm"`); with no client configured (the default) it falls back to `general_enquiry` (confidence 0.5) exactly as before. If the LLM call errors or times out, the result is `LLM_UNCLASSIFIED` (`method="llm_error"`) rather than a guessed category - `main.py` holds that email for manual review (`ApprovalStatus(required=True, status="pending_manual_review")`), suppresses the reply (`response.required=False`), and never recommends a CRM-creating/updating action (`action_engine.FIXED_ACTIONS["LLM_UNCLASSIFIED"]`).
...
No dependencies required (stdlib `unittest`, matching the CLI's zero-dependency design). Covers: the `legal_compliance` gate winning over every other keyword and short-circuiting before any LLM call is attempted, and the LLM fail-safe path - an ambiguous fixture (`AMBIGUOUS_EMAIL`, matches no deterministic rule) paired with an `ExplodingLLMClient` that raises `TimeoutError`, asserting the pipeline returns `LLM_UNCLASSIFIED`, holds for manual review, drafts no reply, and recommends no CRM-touching action.
...
- **Automated tests are still thin**: `scr/tests/test_legal_compliance_and_llm_failsafe.py` covers the `legal_compliance` gate and the LLM fail-safe path (see "Tests" above); everything else was verified by running `python -m scr.main` and spot-checking specific emails (`E001`, `E003`, `E012`, `E004`, `E011`, `E010`) by hand. A day-two version would add `pytest` cases per module, especially the CRM-matching priority order (the exact class of bug that showed up here: an email match on one row must win over a phone match on a different row within the same pass).
- **No LLM-assisted extraction, and classification's LLM hook is unwired by default**: this was a deliberate scope decision - deterministic rules are fully explainable, need no API key, make no external calls, and can't be prompt-injected via email content. `classifier.py` now accepts an optional `llm_client` for ambiguous classification (with a fail-safe `LLM_UNCLASSIFIED` path on error/timeout - see above), but no real client is ever configured in `main.py`, so the CLI still makes zero external calls today. A day-two version could similarly add an LLM as an *optional, sandboxed* second pass for extraction, that only ever proposes values back into the same `ExtractionField(confidence, source)` shape - never free text injected straight into a response - so a human still reviews anything the deterministic layer didn't already handle with confidence.
